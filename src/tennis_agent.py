...
from src.tennis_model import Actor, Critic
...
if torch.cuda.is_available():
    print(f"Cuda available using device 'cuda:0' ")
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed,
                 fc1_units=64, fc2_units=64, fcs1_units=64):
        ...
        self.critic_local = Critic(
            state_size, action_size, random_seed,
            fcs1_units=fcs1_units, fc2_units=fc2_units
            ).to(device)
    ...
    def act(self, state, add_noise=True):
        ...
        if add_noise:
            ...
            # self.eps = max(EPS_END, self.eps*EPSILON_DECAY)
...
class OUNoise:
    ...
    def sample(self):
        ...
        x = self.state
        dx = self.theta * (self.mu - x) + self.sigma * np.random.normal(0, 1, 2)
        self.state = x + dx
        return self.state
